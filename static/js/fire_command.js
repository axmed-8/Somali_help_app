/**
 * Fire Command Center — station-scoped incident desk.
 */
(function () {
  "use strict";

  var boot = window.FIRE_BOOT || {};
  var state = {
  ...
  function typeLabel(t) {
    return String(t || "fire").replace(/_/g, " ");
  }
  ...
  function bindQueueActions(root) {
    ...
    root.querySelectorAll(".q-release").forEach(function (b) {
      b.onclick = function () {
        if (!confirm("Release this case back to the open Fire queue?")) return;
  ...
  function syncDutyBanner() {
    var banner = $("fcc-duty-banner");
    var title = $("fcc-duty-title");
    var sub = $("fcc-duty-sub");
    var meta = $("fcc-duty-meta");
    ...
    if (n > 0) {
      ...
      if (sub) {
        sub.textContent = "fire & accident alerts for " + (state.station.name || "your station");
      }
  ...
  function renderQueue() {
    ...
    fill(
      ...
      "When Call Center alerts or dispatches to fire, cases appear here."
    );
    fill(
      "queue-list",
      otherList.slice(0, 8),
      "Queue is clear",
      "New fire SOS will appear here automatically."
    );
    fill(
      "queue-full-list",
      list,
      "No active Fire incidents",
      "When a citizen or call center routes a case to Fire, it lands in this queue."
    );
    syncDutyBanner();
  }
  ...
  function renderHistory() {
    ...
    if (!state.history.length) {
      el.innerHTML = emptyHtml(
        "No history yet",
        "Completed and cancelled Fire cases will show here."
      );
      return;
    }
  ...
  function mutate(eid, action) {
    api("/api/fire/request/" + eid + "/" + action, { method: "POST", body: {} }).then(function (r) {
  ...
  function afterAcceptOpenChat(eid) {
    var welcome =
      "Fire-ku waa aqbalay xaaladdaada. Halkan nala soo hadal. / We accepted your emergency — message us here.";
  ...
  function chatRowHtml(person) {
    ...
    var preview = meta.lastText
      ? (meta.lastRole === "fire" || meta.lastRole === "admin" ? "You: " : "") + meta.lastText
  ...
  function chatBubbleHtml(m, showCase) {
    ...
    var who = mine ? "FIRE" : m.sender_role === "citizen" ? "Citizen" : esc(m.sender_role || "User");
    var tick =
  ...
  function openDetail(eid) {
    ...
    if (!em) return;
    state.selectedId = eid;
    var modal = $("fcc-detail-modal");
    if ($("fcc-detail-title")) $("fcc-detail-title").textContent = "Incident #" + eid;
    if ($("fcc-detail-body")) {
      $("fcc-detail-body").innerHTML =
        "<p><span class=\"fcc-type-pill " +
        esc(em.type || "") +
        '">' +
        esc(typeLabel(em.type)) +
        "</span> · <strong>" +
    ...
    var actions = $("fcc-detail-actions");
    if (actions) {
      var html =
        '<button type="button" class="hcc-btn hcc-btn-ghost" id="fcc-detail-close">Close</button>';
      if (ACTIVE[em.status]) {
  ...
  function saveStation(e) {
    ...
    api("/api/fire/station", { method: "PUT", body: payload }).then(function (r) {
      if (r && r.success === false) {
        if (err) {
          err.hidden = false;
      ...
      syncStationChips();
      if ($("fcc-brand-name")) $("fcc-brand-name").textContent = state.station.name || "Fire";
      if ($("fcc-op-status")) {
        $("fcc-op-status").textContent = String(state.station.operating_status || "open").replace(
          /^\w/,
          function (c) {
  ...
  function loadAll() {
    ...
    return Promise.all([
      api("/api/get_emergencies?type=fire"),
      api("/api/get_emergencies?type=fire&status=completed,resolved,cancelled"),
      api("/api/notifications"),
      state.stationLinked ? api("/api/fire/station") : Promise.resolve(null),
  ...
  function showPanel(name) {
    ...
    var titles = {
      live: (state.station.name || "fire") + " Command Center",
    ...
    if ($("fcc-page-title")) $("fcc-page-title").textContent = titles[name] || titles.live;
  ...
  function init() {
    ...
    var detailModal = $("fcc-detail-modal");
    if (detailModal) {
